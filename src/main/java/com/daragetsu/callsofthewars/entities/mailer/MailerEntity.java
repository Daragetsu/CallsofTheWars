...
public class MailerEntity extends Monster implements GeoEntity {
    ...
    public void registerControllers(ControllerRegistrar controllers) {
        controllers.add(new AnimationController<>(this, "controller", 2,
                state -> {
                    if (((this.getX() - this.xo) * (this.getX() - this.xo))
                            + ((this.getZ() - this.zo) * (this.getZ() - this.zo)) > 0.0002) {
    ...
    protected void registerGoals() {
        ...
        this.goalSelector.addGoal(3, new Goal() {// can't bother making a new class
            ...
            public boolean canUse() {
                return MailerEntity.this.getTarget() != null && MailerEntity.this.getRandom().nextFloat() < 0.4
                        && MailerEntity.this.distanceToSqr(MailerEntity.this.getTarget()) < 4;
            }
            ...
            public void start() {
                super.start();
                if (MailerEntity.this.level().isClientSide())
                    return;
                ...
                String s2 = Component.Serializer.toJson(
                        Component.literal(String.format("""
                                ...
                                """, MailerEntity.this.getRandom().nextInt(
                                    1000,
                                    696969 // gomennasai
                                ))));
                String s3 = Component.Serializer.toJson(Component
                        .literal("to keep the peace of this world, if you are interested, please ")
                        .append(Component.literal("enlist")
                                .withStyle(style -> style.applyFormat(ChatFormatting.BLUE)
                                        .withClickEvent(new ClickEvent(ClickEvent.Action.RUN_COMMAND, "/enlist"))
                                        .withHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT,
                                                Component.literal("Click to enlist")))))
                ...
                stack.save(tag);
                stack.setTag(tag);
                ItemEntity en = new ItemEntity(MailerEntity.this.level(), MailerEntity.this.getX(),
                        MailerEntity.this.getY(), MailerEntity.this.getZ(), stack);
